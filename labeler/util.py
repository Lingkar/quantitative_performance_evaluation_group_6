...
def select_clean_uncertain(X_train, y_train_noisy, y_clean, quality_model, classifier):
    ...
    for i in np.arange(len(y_predict_label)):
        if (np.argmax(y_train_noisy[i]) == y_predict_label[
            i]):  # or ((np.argmax(y_train_noisy[i]) != y_predict_label[i]) and np.max(predict_prob[i,:]) < 0.8) :
...
def select_clean_noisy(X_train, y_train_noisy, y_clean, y_predict, y_predict_label_second, predict_prob, model):
    ...
    for i in np.arange(len(y_predict[:, 0])):
        if (np.argmax(y_train_noisy[i, :]) == np.argmax(
                y_predict[i, :])):  # or y_predict_label_second[i]== np.argmax(y_train_noisy[i])): #filter 1
...
def al_number_generator(dynamic, num_al, budget, used_budget, predict_prob, loss_old):
    if dynamic == 0:
        ...
        if used_budget < budget:
            ...
            num_al = int(
                np.ceil(num_al * (1 - ((loss_old - loss_new) / loss_new))))  # self.AL-int(np.sign(loss_old - loss_new
...
def inject_noise(dataset, y, noise_level):
    ...
    if dataset == "cifar-10" or dataset == "celeb" or dataset == "mnist":
        ...
        for i in noisy_idx:
            y[i] = np_utils.to_categorical(other_class(NUM_CLASSES[dataset], y[i].argmax(axis=-1)),
                                           NUM_CLASSES[dataset])
...
def stat_func(strong_list, weak_list, clean_list, noisy_list, kmeans_list, y_noisy, y_true):
    ...
    for i in range(0, len(noisy_list)):
        if noisy_list[i] in oracle_list:  ##### Selected Noisy for cleaning
            if np.argmax(y_noisy[noisy_list[i], :]) == np.argmax(
                    y_true[noisy_list[i], :]):  ## Was clean but seelected as noisy
                FN_s += 1
        ...
        if i in kmeans_list:  ##### Selected Noisy and clustered for cleaning
            if np.argmax(y_noisy[noisy_list[i], :]) == np.argmax(
                    y_true[noisy_list[i], :]):  ## Was clean but seelected as noisy in clustering
                FN_c += 1

    return FN_s, FN_d, FN_c


def label_decider(dirty_list_model, y_true, y_pred, pred_prob, n_classes, inf_ind, inf, loss, batch_budget, spent_s,
                  spent_w, limit_w, c, alpha, beta, thr):
    L = loss
    # print("loss:", loss)
    strong_list = []
    weak_list = []
    ...
    for i in range(0, len(inf_ind)):
        ...
        true_label_rank.append(
            n_classes - 1 - np.where(np.argmax(y_true[dirty_list_model[inf_ind[i]], :]) == pred_prob_sort)[0])
        ...
        if Q_s > thr:
            if spent_s + spent_w + c <= batch_budget:
                ...
                # pdb.set_trace()
                yes_flag = y_weak_yn_generator(y_pred[dirty_list_model[inf_ind[i]], :],
                                               y_true[dirty_list_model[inf_ind[i]],
                                               :])  ####### First Ask the Predicted Label
                ...
                if (yes_flag):
                    ...
                    for k in range(0, limit_w):
                        if spent_s + spent_w + 1 <= batch_budget:
                            pred_prob_sort = np.delete(pred_prob_sort,
                                                       np.where(y_ask[-1] == pred_prob_sort)[0])  ####### smart
                            y_ask.append(pred_prob_sort[-1])  ######## smart

                            k = k + 1
                            # pdb.set_trace()
                            yes_flag = y_weak_yn_generator(np_utils.to_categorical(y_ask[-1], n_classes),
                                                           y_true[dirty_list_model[inf_ind[i]], :])
                            spent_w = spent_w + 1
                ...
                reserve_list.append(dirty_list_model[inf_ind[i]]) # add reserve_list to collect the left data from dirty_list_model

        elif spent_s + spent_w + 1 <= batch_budget:
            # pdb.set_trace()
            yes_flag = y_weak_yn_generator(y_pred[dirty_list_model[inf_ind[i]], :], y_true[dirty_list_model[inf_ind[i]],
                                                                                    :])  ####### First Ask the Predicted Label
            y_ask = [np.argmax(y_pred[dirty_list_model[inf_ind[i]], :])]
    ...
    return reserve_list, strong_list, weak_list, spent_s, spent_w, first_yes, weak_questions, np.mean(
        weak_questions), unique_pic, true_label_rank
